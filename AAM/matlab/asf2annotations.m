...
for i = 1 : numel(landmark_files)
  ...
  while size(annotations, 1) < np
    ...
    elseif size(data,2) > 0 && data(1) ~= '#'
	  ...
	  % Fields 3,4 are the relative coordinates, Field 5 is the annotation id. 
	  % The annotations are also transformed into cartesian coordinates.
  ...
  % The image is needed to convert from relative to absolute coordinates
  info = imfinfo(img_path);
	
  scale = [info.Width 0; 0 info.Height];
